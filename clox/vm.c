...
static void runtimeError(const char* format, ...) {
    ...
    for (int i = vm.frameCount - 1; i >= 0; i--) {
        ...
        // Points to the previous instruction which failed
...
static void defineNative(const char* name, NativeFn function) {
    // Must push onto the stack to stop the garbage collector from freeing them
    ...
    if (IS_OBJ(callee)) {
        switch (OBJ_TYPE(callee)) {
            case OBJ_BOUND_METHOD: {
                ...
                // Replace slot 0 of the method CallFrame with the receiver
                // class to let us access it as 'this'
...
static bool invoke(ObjString* name, int argCount) {
    // Grab the class instance from behind the args
    ...
    if (tableGet(&instance->fields, name, &value)) {
        // Replace the class instance with the field value
...
// Return the next byte instruction and advance ip
#define READ_BYTE() (*frame->ip++)

// Return the next 16 byte instruction and advance ip
#define READ_SHORT() \
    (frame->ip += 2, \
...
#define BINARY_OP(valueType, op) \
    ...
    for (;;) {
        ...
        disassembleInstruction(&frame->closure->function->chunk,
            // Convert pointers to int offset
        ...
        switch (instruction = READ_BYTE()) {
            ...
            case OP_SET_PROPERTY: {
                ...
                // At this point we should have the value on the top of the stack, and
                // the instance below that.
                // The instruction contains the name of the field we will set the value to
                ObjInstance* instance = AS_INSTANCE(peek(1));
                tableSet(&instance->fields, READ_STRING(), peek(0));
                // Need to remove the instance but keep the value on the stack
                Value value = pop();
                pop();
                push(value);
                break;
            ...
            case OP_CLASS:
                ...
                break;
            case OP_INHERIT: {
                // Subclass methods will be declared after this so those methods
                // will override any inherited ones
