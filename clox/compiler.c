...
// Store variable name and scope depth for lookup
typedef struct {
    Token name;
    int depth;
    // If this local has been captured by a closure already
    bool isCaptured;
} Local;

// Store the upvalue index and if it's local to this function
typedef struct {
    uint8_t index;
    bool isLocal;
} Upvalue;

// Tells the compiler what kind of function we're currently in
typedef enum {
    TYPE_FUNCTION,
    TYPE_INITIALIZER,
    TYPE_METHOD,
...
typedef struct Compiler {
    // The parent/enclosing compiler - will be NULL if we're at the top level
    // A new compiler is used for each function, so we will have a chain of
    ...
    int localCount;
    // Stores all upvalues using indexes in an array
...
static void emitReturn() {
    if (current->type == TYPE_INITIALIZER) {
        // Init methods should always return the class instance itself
...
// Creates a constant in the current chunk and returns the
// index from the constant table
...
// Finish off the compile with a final return and return the top level function
...
static int addUpvalue(Compiler* compiler, uint8_t index, bool isLocal) {
    ...
    // Check if this upvalue is already in use
...
static int resolveUpvalue(Compiler* compiler, Token* name) {
    // Only top level compiler is not enclosed - must be a global value
    if (compiler->enclosing == NULL) return -1;

    // Check the local variables in the function above this one
    ...
    // Must recursively check all enclosing functions for upvalues
...
static void function(FunctionType type) {
    ...
    // Push all upvalues onto the stack for runtime
...
static void classDeclaration() {
    ...
    if (match(TOKEN_LESS)) {
        ...
        // Lookup superclass and put it on the stack
        variable(false);
...
// Returns pointer to the top level function, or NULL on compile failure
